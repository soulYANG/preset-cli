  ...
  <div class="innerWrap">
    ...
    <el-table :data="tableData" style="width: 100%">
      ...
      <el-table-column prop="address" label="操作">
        <template v-slot="slotScope">
          <i class="el-icon-edit operation" @click="handleEditButtonCliked(slotScope.row)"></i>
...
export default {
  ...
  created() {
    queryAddress(this.pagination).then((res) => {
  ...
  methods: {
    editMethods(){
      ...
      updateAddress(this.editData).then((res) => {
    ...
    handleSwitchChange(item) {
      updateAddressEnable(item).then((res) => {
        console.log(res, "updateAddress");
      });
    },
    saveAddress() {
      ...
      saveAddress(this.postData).then((res) => {
