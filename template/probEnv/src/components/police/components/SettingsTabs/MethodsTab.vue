<template>
  <div class="innerWrap">
    ...
    <PaginationComp :pagination="pagination" @currentChange="handleCurrentPageChange"/>
    ...
    <el-dialog title="新增" :visible.sync="methodsTabVisible">
      ...
      <div class="footer">
        ...
        <el-button type="primary" @click="saveMethods">确 定</el-button>
      </div>
    </el-dialog>
...
export default {
  ...
  created() {
    queryMethods(this.pagination).then((res) => {
      console.log(res, "queryMethods");
  ...
  methods: {
    editMethods() {
      ...
      updateMethods(this.editData).then((res) => {
    ...
    handleDeleteButtonCliked(item) {
      deleteMethods(item.id).then((res) => {
    ...
    handleSwitchChange(item) {
      updateMethodsEnable(item).then((res) => {
        console.log(res, "updateMethods");
      });
    },
    saveMethods() {
      ...
      saveMethods(this.postData).then((res) => {
